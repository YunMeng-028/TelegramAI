...
try:
    from claude_code_sdk import query, ClaudeCodeOptions as SDKOptions
except ImportError:
    # 开发时的模拟实现
    async def query(prompt: str, options: Any = None) -> AsyncGenerator[Dict, None]:
        yield {
            "role": "assistant",
...
class ClaudeCodeSession:
    ...
    def __init__(
        ...
        context: Optional[ClaudeCodeContext] = None
    ):
    ...
    async def send(self, prompt: str) -> AsyncGenerator[ClaudeCodeMessage, None]:
        """发送消息到会话"""
        # 添加用户消息到历史
    ...
    ) -> AsyncGenerator[ClaudeCodeMessage, None]:
        """执行查询"""
        # 构建SDK选项
    ...
    def _build_sdk_options(self) -> Dict[str, Any]:
        ...
        if self.options.allowed_tools:
            options["allowed_tools"] = [tool.value for tool in self.options.allowed_tools]
...
class ClaudeCodeService:
    ...
    async def query(
        ...
        try:
            ...
            raise ClaudeCodeError(f"查询失败: {str(e)}")
    ...
    def _merge_options(self, options: Optional[ClaudeCodeOptions]) -> ClaudeCodeOptions:
        """合并选项"""
        base_options = ClaudeCodeOptions(
            api_key=self.api_key,
            allowed_tools=self.tool_manager.get_allowed_tools()
        )
