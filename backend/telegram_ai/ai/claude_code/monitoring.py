...
class PerformanceMonitor:
    ...
    async def track_operation(self, operation: str, metadata: dict[str, Any] = None):
        ...
        class OperationContext:
            ...
            async def __aexit__(self, exc_type, exc_val, exc_tb):
                duration = time.perf_counter() - self.monitor._start_times.pop(
                    self.op_id
                )
                ...
                if tool_name := self.meta.get("tool"):
                    try:
                        ...
                        self.monitor._update_tool_stats(
                            tool, duration, success, error_msg
                        )
                    except ValueError:
                        pass
                ...
                if duration > self.monitor._get_threshold(self.op_name):
                    logger.warning(f"操作 {self.op_name} 执行时间过长: {duration:.2f}s")
    ...
    def _update_tool_stats(
        self, tool: ClaudeCodeTool, duration: float, success: bool, error: str | None
    ):
        """更新工具使用统计"""
    ...
    def get_statistics(self, time_window: timedelta | None = None) -> dict[str, Any]:
        """获取统计信息"""
        if time_window:
    ...
    def get_tool_statistics(self) -> dict[str, Any]:
        ...
        return {
            "tools": tool_data,
            "most_used": (
                max(tool_data.items(), key=lambda x: x[1]["count"])[0]
                if tool_data
                else None
            ),
            "most_errors": (
                max(tool_data.items(), key=lambda x: x[1]["error_count"])[0]
                if tool_data
                else None
            ),
        }
