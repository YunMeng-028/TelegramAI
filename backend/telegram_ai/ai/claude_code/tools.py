...
DEFAULT_TOOL_PERMISSIONS: List[ToolPermission] = [
    {
        "tool": ClaudeCodeTool.READ,
        "allowed": True,
        "description": "读取文件内容"
    },
    {
        "tool": ClaudeCodeTool.GREP,
        "allowed": True,
        "description": "搜索文件内容"
    },
    {
        "tool": ClaudeCodeTool.GLOB,
        "allowed": True,
        "description": "文件模式匹配"
    },
    {
        "tool": ClaudeCodeTool.TODO_WRITE,
        "allowed": True,
        "description": "任务管理"
    },
    {
        "tool": ClaudeCodeTool.TASK,
        "allowed": True,
        "description": "执行复杂任务"
    },
    {
        "tool": ClaudeCodeTool.WEB_SEARCH,
        "allowed": True,
        "description": "网络搜索"
    },
    {
...
class ToolManager:
    ...
    def __init__(self, custom_permissions: Optional[List[ToolPermission]] = None):
    ...
    def get_allowed_tools(self) -> List[ClaudeCodeTool]:
        """获取允许的工具列表"""
        return [
            tool for tool, perm in self.permissions.items()
    ...
    def get_tool_description(self, tool: ClaudeCodeTool) -> Optional[str]:
        """获取工具描述"""
        perm = self.permissions.get(tool)
        return perm.get("description") if perm else None
        
    def get_scenario_info(self, scenario: str) -> Optional[Dict[str, Any]]:
        """获取场景信息"""
        if scenario not in SCENARIO_PRESETS:
            return None
