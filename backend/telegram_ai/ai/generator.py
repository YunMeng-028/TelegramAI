...
class AdvancedAIGenerator:
    ...
    async def generate_with_tools(self, context: MessageContext) -> str:
        """使用工具增强的生成"""
        # 创建Claude会话
        session = self.claude_service.get_session(session_id=f"chat_{context.chat_id}")
...
"""
    ...
    def _build_claude_context(self, context: MessageContext) -> ClaudeCodeContext:
        ...
        return ClaudeCodeContext(
            ...
            user={"id": context.chat_id, "info": context.user_info},
    ...
    def _detect_emotion(self, text: str) -> str:
        ...
        if any(word in text_lower for word in ["哈哈", "好笑", "搞笑"]):
            return "funny"
        if any(word in text_lower for word in ["开心", "高兴", "快乐"]):
            return "happy"
        if any(word in text_lower for word in ["难过", "伤心", "糟糕"]):
            return "sad"
        if any(word in text_lower for word in ["激动", "兴奋", "太棒"]):
            return "excited"
        if any(word in text_lower for word in ["想", "思考", "考虑"]):
            return "thinking"
        if any(word in text_lower for word in ["爱", "喜欢", "心"]):
            return "love"
        return "happy"
